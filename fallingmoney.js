...
class Circle {
    constructor(x, y, r, dx, dy, str, fll) {
        ...
        this.update = function () {
            if(grav.checked){
                //if perimeter of circle approach the bottom border and vertical velocity is less than 1 
                
                if(canvas.height - (this.y+this.r) < 1 && Math.abs(this.velocity.y) < 1){
                    //stop the movement
                    this.velocity.y = 0;
                    //reset the position of the object to avoid clipping
            ...
            //checks left and right border for bounce
            if (this.x > canvas.width - this.r || this.x < 0 + this.r)
                this.velocity.x = -this.velocity.x;
            //checks top and bottom border for bounce
            if (this.y > canvas.height - this.r || this.y < 0 + this.r){
                this.velocity.y = -this.velocity.y;
                //check if the object hits bottom border and grav is on
            ...
            if(col.checked){
                for (let i = 0; i < arrCircle.length; i++) {
                    ...
                    if(hasCollided(this.x, this.y, this.r, arrCircle[i].x, arrCircle[i].y, arrCircle[i].r)){
                        ...
                        if(arrCircle[i].velocity.y < 3){
                            arrCircle[i].velocity.y = 0;
